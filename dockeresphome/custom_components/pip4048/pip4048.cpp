...
static const char *TAG = "pip4048ups";

void PIP4048Serial::dump_config() {
...
void PIP4048Serial::handle_setup_line_() {
  unsigned long now = millis();
  switch (this->state_) {
   case 1:  // 'Y'
    if (this->line_ != "SM") {
...
void PIP4048Serial::handle_line_() {
  unsigned long now = millis();
  float value;
  switch (this->state_) {
   case 1:  // 'Y'
...
void PIP4048Serial::handle_int_(uint8_t c) {
  switch(c) {
   case '!':
